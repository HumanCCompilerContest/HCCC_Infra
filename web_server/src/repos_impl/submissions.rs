...
impl<'a> Submissions for SubmissionImpl<'a> {
    ...
    async fn get_all_submissions(&self) -> Vec<SubmissionObject> {
        const TARGET_COLUMN: &str = "submits.id, time, asm, error_message, is_ce, error_line_number, result, user_id, name, problem_id, title, statement, code, input_desc, output_desc, problems.score";
